...
Future<UpdateInfo?> checkForUpdate({int proxyPort = 1090}) async {
  try {
    ...
    final baseUrl =
        (customUrl != null && customUrl.isNotEmpty) ? customUrl : appcastUrl;
    ...
    final fetchUrl = baseUrl.contains('drive.usercontent.google.com')
        ? Uri.parse(
            '$baseUrl$cacheBust${DateTime.now().millisecondsSinceEpoch}')
        : Uri.parse(baseUrl);
...
Future<void> downloadAndInstall(
  ...
  try {
    ...
    if (Platform.isMacOS) {
      ...
      // Atomic install: ditto to .new, then mv (with sudo fallback for SIP-enabled Macs)
      await Process.run('sudo', ['-n', 'rm', '-rf', destNew]);
      await Process.run('rm', ['-rf', destNew]); // also try without sudo
      ...
      await Process.run('sudo', ['-n', 'rm', '-rf', dest]);
      final mvResult = await Process.run('sudo', ['-n', 'mv', destNew, dest]);
      if (mvResult.exitCode != 0) {
        // sudo mv failed — try without sudo (SIP disabled or user-owned)
...
# Give /Applications/Lux.app back to user so future updater installs work without sudo
USER_NAME_EARLY=\$(stat -f '%Su' /dev/console 2>/dev/null || echo "\$SUDO_USER")
chown -R "\$USER_NAME_EARLY":staff "\$DEST" 2>/dev/null || true
