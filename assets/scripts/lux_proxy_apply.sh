#!/bin/bash
# lux_proxy_apply.sh <proxy_addr>
# e.g. lux_proxy_apply.sh 127.0.0.1:1090
...
if [ -z "$PROXY_ADDR" ]; then
  echo "Usage: lux_proxy_apply.sh <host:port>" >&2
  exit 1
fi

PROXY="http://$PROXY_ADDR"
NO_PROXY_VAL="localhost,127.0.0.1,10.255.0.1,*.local,169.254/16"
...
launchctl setenv no_proxy    "$NO_PROXY_VAL"  2>/dev/null || true
launchctl setenv NODE_EXTRA_CA_CERTS /Library/Keychains/System.keychain 2>/dev/null || true
launchctl setenv CURL_CA_BUNDLE      /etc/ssl/cert.pem 2>/dev/null || true
launchctl setenv SSL_CERT_FILE       /etc/ssl/cert.pem 2>/dev/null || true
...
printf 'export SSL_CERT_FILE=/etc/ssl/cert.pem # LUX_PROXY\n'    >> /tmp/lux_zshenv_clean
